...
class EventCubit extends Cubit<EventState> {
  ...
  Future<void> modifyEvent(ModifyEventParam modifyEventParam) async {
    ...
    response.fold(
            (failure) => emit(ModifyEventError(
            msg: Constants.mapFailureToMsg(failure))), (modifiedEvent) {
  ...
  Future<void> deleteEvent(int eventId) async {
    ...
    Either<Failure, void> response =
        await eventsUseCases.deleteEvent(eventId);

    response.fold((failure) => emit(DeleteEventError(msg: Constants.mapFailureToMsg(failure))),
            (success) {
